...
Input data is validated on the client side using javascript, and further validated on the server side using PHP. Additionally, when appropriate, data is validated on the database server using triggers (_e.g._ a new question must contain between 1 and 5 categories).
